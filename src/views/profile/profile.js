...
const Profile = () => {
    ...
    return(
        ...
        <div className="profile-container">
            <div className="profile-picture rounded-circle" style={{backgroundImage: `url('${user.image.split('|&&|')[0]}')`}}>
                ...
                <div className="content-panel collapse in" id="my-collapse">
                    {
                        owned.length === 0 ? (
                            ...
                            <Link to={`/detailrecipe?id=${e.id}`} key={e.id} className="profile-recipe-img rounded-2" style={{backgroundImage: `url('${e.image.split('|&&|')[0]}')`}}>
                                <p><b>{e.title}</b></p>
                            </Link>
                        ))
                    }
